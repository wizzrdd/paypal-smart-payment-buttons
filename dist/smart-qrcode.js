window.spbQRCode = function(modules) {
    ...
    function util_base64encode(str) {
        if ("function" == typeof btoa) return btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g, (function(m, p1) {
            return String.fromCharCode(parseInt(p1, 16));
        })));
    ...
    function src_util_uniqueID() {
        ...
        return "xxxxxxxxxx".replace(/./g, (function() {
            return chars.charAt(Math.floor(Math.random() * chars.length));
        })) + "_" + util_base64encode((new Date).toISOString().slice(11, 19).replace("T", ".")).replace(/[^a-zA-Z0-9]/g, "").toLowerCase();
    }
    var util_objectIDs;
    ...
    function util_memoize(method, options) {
        ...
        var memoizedFunction = function() {
            ...
            var cacheKey = util_serializeArgs(args);
            var cacheResult = cache[cacheKey];
            if (cacheResult && cacheTime && Date.now() - cacheResult.time < cacheTime) {
                delete cache[cacheKey];
                cacheResult = null;
    ...
    function util_svgToBase64(svg) {
        return "data:image/svg+xml;base64," + util_base64encode(svg);
    }
    Error;
    ...
    util_memoize((function() {
        ...
        if (uid && "string" == typeof uid) return uid;
        if ((uid = script.getAttribute("data-uid-auto")) && "string" == typeof uid) return uid;
        uid = src_util_uniqueID();
        script.setAttribute("data-uid-auto", uid);
        return uid;
    }));
    function SVG(props) {
        var svg = props.svg, otherProps = _objectWithoutPropertiesLoose(props, [ "svg" ]);
        ...
        var svgProps = _extends({
            src: util_svgToBase64(svg)
        }, otherProps);
        return node_node("img", svgProps);
    }
    function SVGLogo(_ref) {
        var render = _ref.render, name = _ref.name, alt = _ref.alt, logoColor = _ref.logoColor, props = _objectWithoutPropertiesLoose(_ref, [ "render", "name", "alt", "logoColor" ]);
        return node_node(SVG, _extends({}, props, {
            svg: render(),
            alt: "" + (alt || (string = name, string.charAt(0).toUpperCase() + string.slice(1).toLowerCase())),
    ...
    function Logo() {
        ...
        props = _objectWithoutPropertiesLoose(_ref, [ "logoColor" ]), primary = function(name, logoColorMap, logoColor) {
            var colors;
